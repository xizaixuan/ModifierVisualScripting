...
using UnityEngine.Modifier.UIElements;
using UnityEngine.UIElements;

namespace UnityEditor.Modifier.VisualScripting.Editor.Renamable
{
        ...
        static void OnFocusOut(this IRenamable renamable, FocusOutEvent evt)
        {
            ...
            if (!renamable.EditTitleCancelled)
            {
                if (renamable.TitleEditor is TextField textField && renamable.TitleValue != textField.text)
                    renamable.Store.Dispatch(new RenameElementAction((Unity.Modifier.GraphToolsFoundation.Model.IRenamable)renamable.GraphElementModel, textField.text));
            }
            else
            {
                renamable.EditTitleCancelled = false;
